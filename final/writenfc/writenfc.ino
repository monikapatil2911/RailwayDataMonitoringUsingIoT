...
byte block1content[16] = {"Virar"};
byte block2content[16] = {"Palghar"};
...
void loop()
{
  ...
  if ( ! mfrc522.PICC_ReadCardSerial()) {//if PICC_ReadCardSerial returns 1, the "uid" struct (see MFRC522.h lines 238-45)) contains the ID of the read card.
   ...
   //mfrc522.PICC_DumpToSerial(&(mfrc522.uid));//uncomment this if you want to see the entire 1k memory with the block written into it.
         ...
         for (int j=0 ; j<16 ; j++)//print the block contents
         {
           Serial.write (readbackblock2[j]);//Serial.write() transmits the ASCII numbers as human readable characters to serial monitor
         }
         Serial.println("");
